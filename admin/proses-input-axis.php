...
if (!empty($Nama_kartu) && !empty($Nama_produk) && !empty($Kuota_local) && !empty($Kuota_video) && !empty($Kuota_game) && !empty($Kuota_sosmed) && !empty($Kuota_malam) && !empty($Harga) && !empty($Masa_aktif) && !empty($Keterangan)) {
    ...
    $stmt = $koneksi->prepare("INSERT INTO products_paket_axis (Nama_kartu, Nama_produk, Kuota_local, Kuota_video, Kuota_game, Kuota_sosmed, Kuota_malam, Harga, Masa_aktif, Keterangan) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssssss", $Nama_kartu, $Nama_produk, $Kuota_local, $Kuota_video, $Kuota_game, $Kuota_sosmed, $Kuota_malam, $Harga, $Masa_aktif, $Keterangan);
    $query = $stmt->execute();
    ...
    if (!$query) {
        ...
        echo "<script>alert('Data produk axis berhasil ditambah');</script>";
