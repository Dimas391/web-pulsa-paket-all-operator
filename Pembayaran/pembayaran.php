                            ...
                            <div class="mb-3" style="position:absolute; width: 400px; margin-left: 620px; margin-top: -580px; background-color:#eee; padding: 10px; height:">
                                <?php if (!empty($_SESSION["keranjang"])) : ?>
                                    <?php foreach ($_SESSION["keranjang"] as $id => $jumlah) : ?>
                                        <?php
                                        $ambil = $koneksi->query("SELECT * FROM penjualan INNER JOIN products_pulsa ON penjualan.id = products_pulsa.id WHERE penjualan.id = '$id'");
                                        if ($ambil && $ambil->num_rows > 0) {
                                            $row = $ambil->fetch_assoc();
                                            $subharga = $row["harga"] * $jumlah;
                                        ?>
                                            
                                                <p>Nama Produk: <?php echo $row["Jenis_pulsa"]; ?></p>
                                                <p>No Tujuan: <?php echo $row["No_Telepon"]; ?></p>
